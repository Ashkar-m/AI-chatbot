...
Revision ID: e16b595695cd
Revises: 
Create Date: 2024-12-25 00:08:43.155000
...
def upgrade():
    ...
    with op.batch_alter_table('app_details', schema=None) as batch_op:
        batch_op.drop_index('app_details_email_acde8afb_like')

    op.drop_table('app_details')
...
def downgrade():
    ...
    op.create_table('app_details',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    sa.Column('email', sa.VARCHAR(length=254), autoincrement=False, nullable=False),
    sa.Column('submitted_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.PrimaryKeyConstraint('id', name='app_details_pkey'),
    sa.UniqueConstraint('email', name='app_details_email_key')
    )
    with op.batch_alter_table('app_details', schema=None) as batch_op:
        batch_op.create_index('app_details_email_acde8afb_like', ['email'], unique=False)
